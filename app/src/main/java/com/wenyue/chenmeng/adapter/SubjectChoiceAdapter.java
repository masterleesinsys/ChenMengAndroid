 ...
 * 学科选择
 */
public class SubjectChoiceAdapter extends RecyclerView.Adapter<SubjectChoiceAdapter.MyViewHolder> {
    private List<GradeInfo> list;
    private List<StoreProductCategoryInfo> list1;
    private Context context;
    private OnItemClickListener onItemClickListener;
    private int mSelectedPos = -1;  //保存当前选中的position
    private int type = 0;

    public SubjectChoiceAdapter(List<GradeInfo> list, Context context) {
        this.list = list;
        this.context = context;
    }

    public SubjectChoiceAdapter(List<StoreProductCategoryInfo> list1, Context context, int type) {
        this.list1 = list1;
        this.context = context;
        this.type = type;
    }
    ...
    public void onBindViewHolder(final MyViewHolder holder, @SuppressLint("RecyclerView") final int position, List<Object> payloads) {
        if (type == 1) {
            holder.tv_subject.setText(list1.get(position).getName());
        } else {
            holder.tv_subject.setText(list.get(position).getName());
        }
